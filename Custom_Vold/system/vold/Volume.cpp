...
int Volume::formatVol(bool wipe) {
    ...
    if (formatEntireDevice && fstype == NULL) {
        sprintf(diskPath, "/dev/block/vold/%d:%d",
                major(diskNode), minor(diskNode));

        if (initializeMbr(diskPath)) {
    ...
    } else if (strcmp(fstype, "exfat") == 0 || strcmp(fstype, "texfat") == 0 || strcmp(fstype, "vfat") == 0 || strcmp(fstype, "fat") == 0) {
       ...
       fstype = getFsType((const char *)devicePath);
	    ...
	    if (strcmp(fstype, "ext4") == 0 || strcmp(fstype, "ext3") == 0 || strcmp(fstype, "ext2") == 0) {
		    ...
		    setState(Volume::State_Idle);
                ...
                AID_MEDIA_RW, AID_MEDIA_RW, 0007, false)) {
		    errno = EIO;
		    setState(Volume::State_Idle);
		...
		SLOGE("error: Unsupported filesystem on device %s (%s)\n!!!", devicePath, fstype);
                errno = EIO;
                setState(Volume::State_Idle);
                free(fstype);
                return -1;
	    ...
	    SLOGE("error: Can't detect FileSystem on device (%s)\n!!!", devicePath);
            // Unsupported filesystem
            errno = ENODATA;
            setState(Volume::State_Idle);
            free(fstype);
