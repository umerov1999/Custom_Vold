...
int VolumeManager::createAsec(const char *id, unsigned int numSectors, const char *fstype,
    ...
    if (wantFilesystem) {
        ...
        if (usingExt4) {
            ...
            formatStatus = Exfat::format(dmDevice);
        }
        ...
        if (usingExt4) {
            ...
            mountStatus = Exfat::doMount(dmDevice, mountPoint, false, false, false, ownerUid, 0, 0000, true);
        }
...
int VolumeManager::finalizeAsec(const char *id) {
    ...
    if (sb.c_opts & ASEC_SB_C_OPTS_EXT4) {
        ...
        result = Exfat::doMount(loopDevice, mountPoint, true, true, true, 0, 0, 0227, true);
    }
...
int VolumeManager::mountAsec(const char *id, const char *key, int ownerUid, bool readOnly) {
    ...
    if (sb.c_opts & ASEC_SB_C_OPTS_EXT4) {
        ...
        result = Exfat::doMount(dmDevice, mountPoint, readOnly, false, readOnly, ownerUid, 0, 0222, true);
    }
...
int VolumeManager::mountObb(const char *img, const char *key, int ownerGid) {
    ...
    if (Exfat::doMount(dmDevice, mountPoint, true, false, true, 0, ownerGid, 0227, true)) {
